...
module Script
  ...
  class Parser
    ...
    def tokenize(s)
      patterns = [
        [TOKEN_TAG, Regexp.new(/\G(\\[Cehunjcxtqzy*v0123456789fmia!&+-]|\\[sbp][0-9]?|\\w[0-9]|\\_[wqslvVbe+cumna?]|\\__[ctqw]|\\URL)/)],
        [TOKEN_META, Regexp.new(/\G(%month|%day|%hour|%minute|%second|%username|%selfname2?|%keroname|%friendname|%songname|%screen(width|height)|%exh|%et|%m[szlchtep?]|%dms|%j|%c|%wronghour|%\*|%property\[(\\\\|\\\]|(?!\\\\|\\\]).)+?\])/)],
        [TOKEN_NUMBER, Regexp.new(/\G[0-9]+/)],
        [TOKEN_OPENED_SBRA, Regexp.new(/\G\[/)],
        [TOKEN_CLOSED_SBRA, Regexp.new(/\G\]/)],
        [TOKEN_STRING, Regexp.new(/\G(\\\\|\\%|\\\]|[^\\\[\]%0-9])+/)],
        [TOKEN_STRING, Regexp.new(/\G[%\\]/)],
      ]
      tokens = []
      pos = 0
      end_ = s.length
